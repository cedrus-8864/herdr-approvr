...
# `herdr plugin uninstall cedrus.prompt-reply`, because uninstalling a
...
Two things this script cannot do for you:
  ...
  * The "Herdr Prompt Reply" row in System Settings > Notifications outlives the
    bundle. macOS prunes it on its own schedule; there is no supported command
    to delete one entry.
  * Your plugin config still exists. Remove it with:
      rm -rf "$(herdr plugin config-dir cedrus.prompt-reply)"
    (run it before uninstalling the plugin, while the id still resolves)

If ANOTHER copy of HerdrPromptReply.app exists (e.g. a GitHub-managed install next
...
authorization both copies share. Re-register the surviving copy:
  lsregister -f <path-to-other>/assets/HerdrPromptReply.app
